...
const ENANO = {
  ...
  escenas: {
    ...
    invierno: { titulo: "El mercado de invierno",
      texto: (pj) => `Lord Hargrim Oroviejo, dueño de los Cascos, baja al vestuario por primera vez desde el descenso, con un contrato bajo el brazo: quiere "modernizar", que en boca de un mecenas quiere decir vender. Esa misma semana juegas un amistoso contra las Espinas de Cythel, elfos, y su corredor —Faelas, el más rápido que has visto que no seas tú— te espera al acabar fuera del vestuario de piedra con dos jarras, que en un elfo educado es una provocación cortés. "En mi equipo", dice, "correr es lo normal. Nadie me grita 'aquí no se corre', nadie me manda a tapar un hueco en una esquina. ¿Por qué te quedas con ellos?" ${pj.flags.corristeEnElDerbi ? "Vio tu touchdown del derbi en la Cristalvisión. 'Corres como yo', dice. 'Podrías correr donde te aplaudan'." : ""}`,
      opciones: [
        { txt: "Coger la jarra del elfo. Escuchar al que corre sin culpa.", fx: { Honor: 1, Astucia: 1, rel: { faelas: 3, dorin: -1 }, flag: "manoAFaelas" }, msg: "Bebes con el rival. Los tuyos, si se enteran, mirarán la pared. Faelas sonríe: 'Sabía que eras de los míos y no lo sabías'. Odias que suene a verdad." },
        { txt: "Tirarle la jarra a los pies. No bebes con quien hundió a tu club.", req: { Honor: 2 }, forzable: true, fx: { Ferocidad: 1, Voluntad: 1, rel: { dorin: 2, durak: 2, faelas: -2 }, flag: "contraFaelas" }, msg: "'Los tuyos nos bajaron bailando', dices. 'Prefiero subir de pie con la caja que bajar bonito contigo'. Faelas recoge su jarra sin ofenderse; los elfos no se ofenden. 'Como quieras. Nos vemos en el campo, corredor de esquina'. Duele el mote." },
        { txt: "Escucharle y no decir nada. Guardarte la jugada y el desprecio.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, flag: "guardasteLaJugada" }, msg: "Le dejas hablar y no le das ni el sí ni el no. Faelas se va sin saber de qué lado estás, que es justo donde tú tampoco lo sabes." },
      ] },

    /* ---------- CAPÍTULO 2: EL DESCENSO ---------- */
    faelasLlega: { titulo: "El corredor del otro banquillo",
      texto: (pj) => `Faelas juega en las Espinas de Cythel, y esta semana os toca contra ellos en liga. La víspera te busca otra vez, con dos jarras, y te dibuja con el dedo en la piedra una jugada donde el corredor y el receptor se cruzan por fuera de la caja. Es bonita. Es lo contrario de todo lo que Durak te enseña, y es la jugada con la que su equipo os bajó a Segunda. ${pj.flags.manoAFaelas ? "Te la enseña porque bebiste con él, corredor a corredor." : pj.flags.contraFaelas ? "Te la enseña aunque le tiraste la jarra, porque eres el único del otro bando que la entendería." : ""}`,
      opciones: [
        { txt: "Quedarte la jugada. Aprender del enemigo, aunque sea del enemigo.", fx: { Astucia: 1, stat: { AG: 1 }, rel: { faelas: 2, durak: -1 }, flag: "jugadaFaelas" }, msg: "Te la aprendes de memoria: una jugada es una jugada, venga de quien venga. La corres solo al amanecer, en el campo vacío, sin que Durak lo vea. Un día la usarás. Contra él, o como él. Todavía no lo sabes." },
        { txt: "Decirle que en Baraz-Ankor eso no se juega.", fx: { Honor: 1, rel: { faelas: -2, dorin: 1 } }, msg: "Faelas asiente. 'Ya'. Se vuelve a su banquillo. Cuando la caja se rompa, y se romperá, te acordarás del dibujo en la piedra." },
        { txt: "Llevarle la jugada a Durak. Para saber cómo se defiende.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, rel: { durak: 1, faelas: -1 }, flag: "jugadaRobada" }, msg: "Durak mira el dibujo del cruce. 'Es de elfo', dice, y por una vez no lo borra: lo estudia, para saber por dónde entran. Aprender del enemigo para pararlo también es la caja." },
      ] },
    elfos: { titulo: "Los que no chocan", partido: { rival: "Las Espinas de Cythel", fuerza: 3 },
      texto: (pj) => `Las Espinas de Cythel no chocan. Nunca. Su receptora salta por encima de la caja, su lanzador tira desde la otra mitad del campo y sus linieros bailan alrededor de Dorin hasta que se le acaban los turnos. Fue así como los Cascos bajaron. Durak alinea la caja de siempre. ${pj.flags.jugadaFaelas ? "Enfrente, Faelas corre el mismo cruce que te dibujó, y cada vez que os adelanta sonríe. Sabes cómo va: podrías responderle con lo suyo." : "Enfrente, Faelas corre, y cada vez que te adelanta sonríe."} Turno seis. {marcador}. Dorin, dentro, grita la casilla siguiente. Helgra, desde el banquillo, señala la banda con la jarra.`,
      opciones: [
        ...
        { txt: "Salir de la caja y correr tú el cruce que le aprendiste a Faelas.", req: { flag: "jugadaFaelas" }, forzable: true, tirada: { stat: "AG", obj: 9, riesgo: true,
          ok: { txt: "Sales de la caja y corres el cruce del elfo contra el elfo: por fuera, donde Cythel no espera que un enano llegue. Les ganas la carrera con su propia jugada. Dos touchdowns en cuatro turnos. Dos a dos. Faelas, enfrente, por primera vez no sonríe. El estadio de piedra no sabe cómo se aplaude a un enano que corre como un elfo. Aprende.", fx: { fama: 12, Astucia: 1, gol: 1, rel: { faelas: 3, helgra: 2, aficion: 3, durak: -3, dorin: -2 }, flag: "cruceEnCythel" } },
          ko: { txt: "Sales a correr el cruce y Faelas, que lleva doscientos años corriéndolo, te deja atrás con un quiebro que no habías visto. Cruza él. Tres a cero, y Durak no te sienta, que es peor que si lo hiciera.", fx: { golRival: 1, rel: { durak: -3, dorin: -2 } } } } },
    ...
    descenso: { titulo: "El partido del descenso", partido: { rival: "Los Cascos Rotos de Karag", fuerza: 3, tipo: "remontada" },
      ...
      opciones: [
        ...
        { txt: "Pasar corto a un liniero veterano y abrirle tú el hueco.", tirada: { stat: "ST", obj: 9, riesgo: false,
          ok: { txt: "Un pase de enano, corto y feo, a un liniero de ciento sesenta años, y tú por delante abriendo el hueco a hombros. Cruza él, protegido, mientras tú aguantas al minotauro. Dos a uno. Los Cascos no bajan, y el touchdown lo ha hecho la caja, no el que corre. Dorin, desde la camilla si sigue, asiente.", fx: { fama: 8, Honor: 1, gol: 1, rel: { dorin: 2, club: 3, aficion: 2 }, flag: "salvasteALosCascos" } },
          ko: { txt: "El pase corto se pierde en el barro entre botas viejas. Lo recoge un Casco Roto y cruza. Uno a dos. Tercera. La caja no perdona un pase que no era suyo.", fx: { golRival: 1, rel: { club: -1 }, flag: "descendisteis" } } } },
      ] },
    ...
    losJovenes: { titulo: "Los que quieren correr",
      ...
      opciones: [
        ...
        { txt: "Entrenar con ellos el cruce que le viste a Faelas, a escondidas.", req: { flag: "jugadaFaelas" }, forzable: true, fx: { Astucia: 1, Ambición: 1, rel: { dorin: -2, durak: -2 }, flag: "jovenesCorren" }, msg: "Cuatro enanos jóvenes, al amanecer, en el campo vacío, corriendo la jugada del enemigo hasta que sale. Helgra os pone la cerveza sin decir nada. Dorin lo sabe. Durak lo sabe. Nadie dice nada todavía." },
    ...
    laVenta: { titulo: "Los Grifos de Norburgo",
      ...
      opciones: [
        ...
        { txt: "Entrenar solo, como corría Faelas.", fx: { Voluntad: 2, rel: { faelas: 1, club: -1 } }, msg: "Corres alrededor del campo antes de que lleguen los humanos y te vas después. Entiendes ahora al elfo que corría solo mientras su propia gente no le entendía. Dos raros, cada uno en su bando, corriendo al amanecer en campos vacíos a leguas de distancia." },
      ] },
    ...
    brokk: { titulo: "La carta de Brokk",
      texto: (pj) => `Llega una carta de Baraz Kadrin con la letra apretada de Brokk. Dice que los Yunques han visto el partido de los Toros en la Cristalvisión, que ${pj.flags.rodillaDeNorburgo ? "vieron cómo te rompían la rodilla sin caja" : "vieron cómo corrías sin caja"}, y que el vestuario entero se puso en pie, no sabe si de orgullo o de vergüenza. Dice que Dorin ha vuelto a jugar. Dice que los Cascos van últimos. Al final, con otra tinta: 'Vuelve. Corre allí donde te cubran'.${pj.rel.brokk >= 3 ? " La letra, en esa última línea, se suelta un poco, que en Brokk es lo más parecido a un abrazo que sabe escribir." : pj.rel.brokk <= 0 ? " Es corta y seca, letra de hermano al que dejaste con el hueco sin tapar." : ""}`,
      opciones: [
    ...
    capitania: { titulo: "El brazalete",
      ...
      opciones: [
        ...
        { txt: "Coger el brazalete y ponérselo a Nain. El que tapa huecos, no el que corre.", req: { rel: ["dorin", 2] }, forzable: true, fx: { Honor: 2, rel: { club: 2, dorin: 1 }, flag: "brazaleteANain" }, msg: "Se lo pones a Nain, el joven que aprendió a correr contigo y a cubrir con Brokk. 'Un capitán es el que se queda', le dices, 'no el que se va'. Nain no sabe qué decir. Dorin sí: por primera vez te mira como a un enano que entendió la caja. Tú diriges igual, sin brazalete. Pesa menos y manda igual." },
      ] },
    ...
    ascenso: { titulo: "El partido del ascenso", partido: { rival: "Las Espinas de Cythel", fuerza: 3 },
      texto: (pj) => `Último partido. Ganar es Primera, trescientos años después de la última vez que los Cascos tuvieron que subir a nada. Contra Cythel, los que no chocan, los que os bajaron. ${pj.flags.grimnirMurio ? "Sin Grimnir. Su hacha está en el banquillo, con Helgra." : "Grimnir está suelto, con el hacha que no puede llevar en la mirada."} ${pj.flags.cajaQueSeAbre ? "La pizarra de Durak tiene tu caja, la que se abre en el turno ocho." : pj.flags.abristeLaCaja ? "La pizarra de Durak está vacía. La caja la dibujas tú cada partido." : "La pizarra de Durak tiene la caja de siempre."} Turno ocho. {marcador}. La caja llega a la mitad del campo y los elfos bailan alrededor. Faelas corre enfrente, con la hoja de Cythel, y te busca con la mirada: quien cruce esta jugada sube a Primera. Nain está en la esquina, mirándote.`,
      opciones: [
        { txt: "Abrir la caja en el turno ocho. Tú por la banda, Nain por dentro, Skalf al fondo.", req: { flag: "cajaQueSeAbre" }, forzable: true, tirada: { stat: "AG", obj: 9, riesgo: true,
          ok: { txt: "La caja se abre como en la pizarra. Los elfos, que esperaban una caja, se encuentran cuatro enanos corriendo por sitios distintos. Bailan hacia el sitio equivocado. Anotáis dos. Tres a uno. Primera. Faelas, enfrente, entiende que ha perdido contra algo que él no puede copiar: una caja que corre. El estadio de piedra golpea el suelo tan fuerte que la Cristalvisión pierde el cristal.", fx: { fama: 20, Astucia: 2, gol: 1, rel: { durak: 4, club: 5, aficion: 5 }, flag: "ascendisteis" } },
        ...
        { txt: "Pasar a Nain. Un pase largo, de enano, feo, al fondo del campo.", req: { rel: ["club", 2] }, forzable: true, tirada: { stat: "AG", obj: 10, riesgo: false,
          ok: { txt: "Un pase de cuarenta metros lanzado por un enano. Sale como sale: bajo, sin gracia, con efecto de piedra. Nain, el joven que aprendió a correr contigo, lo caza en carrera y cruza. Dos a uno. Primera, y el touchdown del ascenso lo ha hecho un enano que corre, formado por otro enano que corre. La montaña cría a los suyos.", fx: { fama: 15, Honor: 1, gol: 1, rel: { club: 4, aficion: 3, durak: 1 }, flag: "ascendisteis", flags: ["nainAscenso"] } },
          ko: { txt: "Un pase de cuarenta metros lanzado por un enano. Sale como sale. Lo intercepta un elfo de Cythel que ni siquiera salta. Uno a dos. Segunda. Faelas te busca al acabar, enfrente: 'Otra vez el año que viene. Con más efecto'.", fx: { golRival: 1, rel: { faelas: 1 } } } } },
      ] },
    ...
    elfoOEnano: { titulo: "Lo que es Faelas",
      texto: (pj) => `Faelas ha tenido una carrera larga —los elfos las tienen—, y las Hojas de Ellorien, el equipo de la corte, le ofrecen volver al bosque por lo que gana un mecenas. Te lo cuenta antes de la final, y por primera vez no suena a burla: suena a un corredor viejo al que su gente, por fin, llama a casa. "Tú volviste a tu montaña", te dice. "Yo nunca me fui de la mía del todo, pero me trataron como a ti te trataban: como al raro que corre. Ahora me llaman héroe. ¿Voy? ¿O me quedo siendo el elfo que jugó contra enanos y aprendió a respetarlos?" Te pregunta a ti, porque eres el único que sabe lo que es correr donde no te quieren.`,
      opciones: [
        { txt: "Pedirle que juegue la final con vosotros. Un raro entiende a otro raro.", req: { rel: ["faelas", 3] }, forzable: true, fx: { Honor: 2, rel: { faelas: 4, aficion: 1 }, flag: "faelasSeQueda" }, msg: "'Ven a jugar una con nosotros', dices. 'Contra los que te rechazaron por jugar como jugabas'. Faelas se ríe, y por una vez no es una sonrisa élfica de superioridad: es la de alguien a quien acaban de entender. Se pone el casco de hierro para la final. Once enanos golpean el suelo con los pies, despacio, que es lo que hacen cuando no saben aplaudir a un elfo." },
        { txt: "Decirle que vuelva al bosque. Uno de los dos debe llegar a casa.", fx: { Honor: 1, rel: { faelas: 2, dorin: 1 }, flag: "faelasSeVa" }, msg: "'Vuelve', le dices. 'Uno de los dos tiene que poder volver a casa sin que le griten'. Faelas te mira largo, con esa cara élfica que no envejece. 'Nos vemos en la final, entonces. Con la hoja verde. Por última vez'. Jugará contra ti, en su bando por fin." },
        { txt: "Que decida el vestuario. Que voten si un elfo se pone el casco de hierro.", req: { flag: "helgraMaestra" }, forzable: true, fx: { Astucia: 1, rel: { helgra: 2, club: 2, faelas: 1 }, flag: "faelasVotado", flags: ["faelasSeQueda"] }, msg: "Helgra pone once jarras. Diez se levantan por que el elfo juegue una final con vosotros. Dorin no levanta la suya: 'No hace falta votar lo que es'. Faelas se pone el casco y bebe de la jarra de Dorin." },
      ] },
...
const ORCO = {
  ...
  escenas: {
    ...
    halflings: { titulo: "Un campo con nabos",
      ...
      opciones: [
        { txt: "Retar a los halflings. Quien pierde se va del campo.", fx: { Honor: 2, Ferocidad: 1, rel: { banda: 2, aficion: 1 }, eje: { bandaCharca: 1 }, flag: "retasteHalflings" }, msg: "Los halflings aceptan porque son halflings: creen que perder es que se acabe la comida. Snotlig dice que es una tontería. Ma Gorka apuesta contra ti. Es la primera vez que alguien apuesta sobre tu banda." },
        ...
        { txt: "Pedir el oro a Ma Gorka. Al doscientos por ciento.", fx: { Astucia: 1, oro: -20, rel: { gorka: 2, snotlig: -1 }, eje: { deudaLibre: 1 }, flag: "deudaGorka" }, msg: "Ma Gorka te presta. Te apunta en una pared con una uña: 'El pequeño. Doscientos'. Compras el campo a los halflings, que lo venden llorando y comiendo. Tienes campo y tienes deuda. La segunda crece más rápido." },
      ] },
    ...
    maGorka: { titulo: "Ma Gorka",
      ...
      opciones: [
        { txt: "Nada. Da Banda no debe.", req: { Voluntad: 2 }, forzable: true, fx: { Voluntad: 2, Honor: 1, rel: { gorka: 1, banda: 1 }, eje: { deudaLibre: -2 }, flag: "sinDeuda" }, msg: "Ma Gorka se ríe con toda la cueva. 'Todos deben'. No te apunta en la pared. Es la única banda de Sexta que no está en su pared, y eso, en Sexta, es un nombre." },
        { txt: "Lo justo para cascos de verdad. Y pagarlo con victorias.", fx: { Astucia: 1, oro: 40, rel: { gorka: 2, banda: 2 }, eje: { deudaLibre: 1 }, flag: "deudaGorka" }, msg: "Te presta. Te apunta. Los goblins estrenan cascos que no le han robado a nadie y no saben ponérselos. Ganáis dos partidos y Ma Gorka cobra el primero entero." },
    ...
    wazzok: { titulo: "El chamán que se equivoca",
      ...
      opciones: [
        { txt: "Quedártelo. Un chamán que falla es mejor que ninguno.", fx: { Astucia: 1, rel: { wazzok: 3, banda: 1 }, eje: { enteraSolo: 1 }, flag: "wazzokEnLaBanda" }, msg: "Se queda. Predice tu muerte cada domingo durante años. Falla cada domingo. Los goblins empiezan a creer que mientras Wazzok falle, no puedes morir. Tú también, un poco." },
        { txt: "Echarlo a bastonazos con su propio bastón.", fx: { Ferocidad: 1, rel: { wazzok: -3, snotlig: 2 }, eje: { enteraSolo: -1 }, flag: "wazzokEchado" }, msg: "Le echas. Se va cojeando y prediciendo. Snotlig está contento. El domingo siguiente, sin chamán, pierdes por primera vez en la liga. Es casualidad. Wazzok, desde lejos, no lo cree." },
    ...
    elPuente: { titulo: "Bajo el puente",
      ...
      opciones: [
        ...
        { txt: "Ir con la banda entera. Que vea que somos muchos.", fx: { Ferocidad: 1, rel: { banda: 2, grimgutz: 1 }, eje: { enteraSolo: -1 }, flag: "perdisteUnGoblin" }, msg: "Vais los seis. Grimgutz sale de debajo del puente, mira a cinco goblins y un orco, y se come al goblin más cercano. Snotlig te mira. Es el primero de los tuyos que pierdes, y lo has perdido por un fichaje." },
    ...
    convencer: { titulo: "Cómo se convence a un troll",
      ...
      opciones: [
        ...
        { txt: "Pegarle hasta que entienda quién manda.", req: { Ferocidad: 3 }, forzable: true, tirada: { stat: "ST", obj: 10, riesgo: true,
          ok: { txt: "Le pegas. Un orco pegándole a un troll. Grimgutz se sorprende tanto que se sienta. Le pegas otra vez. Se ríe, que en un troll es un terremoto. Entiende. Manda el pequeño. La banda entera lo ha visto.", fx: { Ferocidad: 2, fama: 5, stat: { ST: 1 }, rel: { grimgutz: 2, banda: 3 }, eje: { jefeIgual: 1 }, flag: "pegasteAGrimgutz" } },
    ...
    skabnik: { titulo: "El fanático",
      ...
      opciones: [
        ...
        { txt: "Ponerle una cadena más corta.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, rel: { skabnik: 2, banda: 2 }, eje: { enteraSolo: 1 }, flag: "cadenaCorta" }, msg: "Le acortas la cadena. Gira más despacio, tumba menos, y no alcanza a los goblins. Skabnik llora: era su cadena. Pero no llora ninguno de los tuyos. Snotlig te mira como se mira a un jefe." },
    ...
    snotligYElTroll: { titulo: "Dónde duerme un goblin",
      ...
      opciones: [
        { txt: "Empujar a Snotlig hacia el hueco. Con cuidado.", fx: { Honor: 2, rel: { snotlig: 3, grimgutz: 2, banda: 2 }, eje: { enteraSolo: 1 }, flag: "snotligDurmioConElTroll" }, msg: "Le empujas. Snotlig se queda rígido en el hueco caliente, con los cascos puestos, esperando que le coman. No le comen. Se duerme. Grimgutz ronca. Desde esa noche Snotlig duerme ahí, y el troll no se mueve hasta que el goblin se levanta." },
        { txt: "Dormir tú en el hueco. Que Snotlig vea que no come.", fx: { Voluntad: 1, rel: { grimgutz: 3, snotlig: 1 }, eje: { enteraSolo: 1 }, flag: "dormisteConElTroll" }, msg: "Duermes en el hueco. Es lo más caliente que has dormido. Snotlig lo ve desde fuera y al tercer día se acerca. Al cuarto duerme a tu lado, con el troll detrás de los dos. Es una banda apretada." },
        { txt: "Dejarle temblar. Un goblin que no confía en su troll no dura.", fx: { Ferocidad: 1, Honor: -1, rel: { snotlig: -2, grimgutz: 1 }, eje: { enteraSolo: -1 }, flag: "dejasteTemblarASnotlig" }, msg: "Le dejas. Snotlig tiembla toda la noche y no se acerca nunca al troll. En el campo, tampoco: le deja solo. Grimgutz se sienta encima de quien tiene la bola sin nadie que le diga quién." },
      ] },
    ...
    elGoblinMuerto: { titulo: "Grot",
      ...
      opciones: [
        { txt: "Sentarte con Snotlig hasta que amanezca.", fx: { Honor: 3, Voluntad: 1, rel: { snotlig: 4, banda: 3 }, eje: { enteraSolo: 1 }, flag: "velasteAGrot", flags: ["grotHonrado"] }, msg: "Te sientas. No habláis. Al amanecer Snotlig dice: 'Era el que te sacó del cesto'. No lo sabías. Enterráis a Grot bajo el puente, con su casco, que le sigue tapando los ojos." },
        { txt: "Echar a Skabnik esa misma noche.", req: { flag: "skabnikEnLaBanda" }, forzable: true, fx: { Honor: 2, rel: { snotlig: 3, skabnik: -4, banda: 2, aficion: -1 }, eje: { enteraSolo: 1 }, flag: "skabnikEchadoPorGrot", flags: ["grotHonrado"] }, msg: "Echas a Skabnik girando hacia el camino. Perdéis un arma. Ganas tres goblins que vuelven a dormir cerca. Snotlig te mira como no te miraba desde que eras más pequeño que él." },
        { txt: "Colgar el casco de Grot en el poste. Y jugar el domingo.", fx: { Ferocidad: 1, Voluntad: 2, rel: { banda: 2, snotlig: 1, aficion: 1 }, eje: { enteraSolo: 1 }, flag: "cascoDeGrot", flags: ["grotHonrado"] }, msg: "Cuelgas el casco. El domingo, cada uno de los tuyos lo toca al salir. Ganáis. Es lo que hacen los orcos con los muertos: jugar el domingo. Snotlig lo toca el último." },
      ] },
    snotligReta: { titulo: "Lo que se cobra un goblin",
      ...
      opciones: [
        { txt: "Decirle que lo sabes. Delante de los goblins, mañana.", fx: { Honor: 3, rel: { snotlig: 5, banda: 3 }, eje: { jefeIgual: -2 }, flag: "reconocisteASnotlig", flags: ["snotligMitad"] }, msg: "Lo dices al día siguiente delante de todos: 'La banda la hizo Snotlig'. Los goblins no saben qué hacer con un orco que dice eso. Snotlig sí: se sienta a tu derecha para siempre. Un jefe con un goblin a la derecha. Gorgomor no lo entendería." },
        { txt: "Retarle. Que se sepa quién manda, de una vez.", req: { Ferocidad: 3 }, forzable: true, tirada: { stat: "ST", obj: 7, riesgo: false,
          ok: { txt: "Le retas y gana quien tenía que ganar. Snotlig acaba en el suelo, riéndose mal. 'Ya', dice. Se queda en la banda. A tu izquierda. Es distinto que a la derecha.", fx: { Ferocidad: 1, rel: { snotlig: 1, banda: 2 }, eje: { jefeIgual: 2 }, flag: "retasteASnotlig", flags: ["eresElJefe"] } },
          ko: { txt: "Le retas y Snotlig, que lleva treinta años robando cascos, te tira un casco a la cara y te tumba con el pie. Un goblin tumbando a un orco. Los otros tres lo ven. 'Ya', dice, y se va a dormir a tu derecha.", fx: { Voluntad: 1, rel: { snotlig: 2, banda: 1 }, eje: { jefeIgual: 1 }, flags: ["eresElJefe"] } } } },
        { txt: "Darle la mitad. De todo. Para siempre.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, oro: -20, rel: { snotlig: 4, gorka: 1 }, eje: { jefeIgual: -1 }, flag: "mitadASnotlig", flags: ["snotligMitad"] }, msg: "'La mitad', dices. Snotlig lo piensa y escupe. Trato. Un orco y un goblin a medias. Ma Gorka, cuando se entera, sube las apuestas: nunca ha visto una banda con dos cabezas que dure. Dura." },
      ] },
    ...
    laDeuda: { titulo: "El doscientos por ciento",
      ...
      opciones: [
        { txt: "Pagar lo que se debe y nada más. Con victorias.", req: { Voluntad: 3 }, forzable: true, fx: { Voluntad: 2, Honor: 2, oro: -40, rel: { gorka: 1, banda: 2 }, eje: { deudaLibre: -2 }, flag: "pagasteAGorka" }, msg: "Pagas. Hasta la última corona, con lo que gana la banda los domingos. Ma Gorka te tacha de la pared con la uña. Es la primera vez que tacha a alguien. La cueva entera lo ve." },
        { txt: "Darle la mitad. Una ogra en la banda es una ogra en el campo.", fx: { Astucia: 1, Honor: -1, oro: 80, rel: { gorka: 4, banda: -1, snotlig: -2 }, eje: { deudaLibre: 2 }, flag: "gorkaEnLaBanda" }, msg: "Le das la mitad. Ma Gorka ficha cascos, apotecario y un chamán que no falla, y los domingos se sienta en la banda con tres metros de silencio. Ganáis más. Snotlig dice que ya no es la banda. Tiene razón a medias." },
        { txt: "Robarle a Rey Krug para pagarle a Ma Gorka. Es lo que haría Snotlig.", req: { rel: ["snotlig", 3] }, forzable: true, tirada: { stat: "AG", obj: 9, riesgo: true,
          ok: { txt: "Snotlig y tú entráis en Gorgomor de noche, robáis los cascos de los Rompecráneos, los del Cáliz, y se los vendéis a Ma Gorka por lo que debéis. Ma Gorka paga y no pregunta. Rey Krug, al día siguiente, pregunta. Mucho.", fx: { Astucia: 2, Honor: -2, oro: 60, rel: { snotlig: 3, gorka: 2 }, eje: { deudaLibre: -1, bandaCharca: -1 }, flag: "robasteAKrug" } },
    ...
    vender: { titulo: "Vender al troll",
      ...
      opciones: [
        { txt: "No venderlo. Grimgutz es de la banda.", fx: { Honor: 3, rel: { grimgutz: 5, banda: 2, gorka: -2, snotlig: -1 }, eje: { enteraSolo: 1 }, flag: "grimgutzSeQueda" }, msg: "No lo vendes. Grimgutz no se entera de nada. Esa noche se sienta a tu lado en el río, como en el puente, y os quedáis mirando el agua. Vale un campo entero y mira moscas. Es tuyo." },
        { txt: "Venderlo. Con el oro, cascos, apotecario y un año sin deber nada.", fx: { Astucia: 1, Honor: -2, oro: 100, rel: { grimgutz: -5, banda: -2, gorka: 2, snotlig: 2 }, eje: { enteraSolo: -2, deudaLibre: -1 }, flag: "grimgutzVendido" }, msg: "Lo vendes. Se lo llevan con cuerdas y con una vaca de cebo. Se gira una vez, en el camino, y te mira sin entender. Los Diente-rotos ganan cuatro partidos seguidos. Da Banda, ninguno. El oro compra cascos. No compra lo otro." },
    ...
    wazzokAcierta: { titulo: "El domingo que acierta",
      ...
      opciones: [
        { txt: "Sentarle en el banquillo. Que no juegue. Que no muera.", fx: { Honor: 2, rel: { wazzok: 3, banda: 1 }, eje: { enteraSolo: 1 }, flag: "wazzokEnElBanquillo", flags: ["wazzokHonrado"] }, msg: "Le sientas. Wazzok protesta con el bastón. Juega la banda sin chamán y gana. Al acabar, Wazzok sigue en el banquillo, vivo, con cara de haber fallado otra vez. Es la primera vez que le alegra fallar. Predice tu muerte para el domingo siguiente, con voz nueva." },
        { txt: "Dejarle jugar. Un chamán decide sus domingos.", fx: { Voluntad: 1, rel: { wazzok: 2, banda: 2 }, eje: { enteraSolo: -1 }, flags: ["wazzokMurio"], flag: "wazzokJugo" }, msg: "Juega. En el turno nueve señala a un rival con el bastón, Grimgutz va, y en el hueco que deja el troll un blitzer pisa a Wazzok. No se levanta. Acertó. Los goblins entierran las setas con él. Grimgutz no vuelve a ir a donde nadie señala: se sienta encima de quien tiene la bola, que es lo que sabía antes." },
        { txt: "Preguntarle qué ve. Antes.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, rel: { wazzok: 3 }, eje: { enteraSolo: 1 }, flag: "loQueVioWazzok", flags: ["wazzokHonrado"] }, msg: "'Un elefante muerto', dice, 'y tú encima. Y yo no'. Le sientas en el banquillo. Vive. Cuando llegues a Gorgomor y veas el elefante, sabrás que acertó dos veces. Es más que cualquier chamán." },
      ] },
    ...
    invitacion: { titulo: "Gorgomor invita",
      ...
      opciones: [
        ...
        { txt: "No ir. Mandar el hueso de vuelta con algo rascado.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 2, fama: 8, rel: { aficion: 2, snotlig: 2 }, eje: { bandaCharca: 2 }, flag: "noFuiste", flags: ["mandasteElHuesoDeVuelta"] }, msg: "Rascas en el hueso: 'Ven tú'. Krug lo lee. Gorgomor lo lee. Al mes, los Rompecráneos bajan a tu campo de nabos, con el Cáliz, porque un Jefe Supremo no puede no ir. Trampa al revés." },
      ] },
    ...
    ultimoPartido: { titulo: "El último partido", partido: { rival: "Los Cuatro Dedos", fuerza: 3 },
      ...
      opciones: [
        ...
        // Snotlig, cincuenta anios, puede jugar su ultimo domingo — y no volver.
        // Es su eleccion (la opcion lo dice) y una buena muerte de orco, sin castigo
        // moral (biblia 18): a pulso decides que corra una vez mas. La marca de estado
        // es snotligMuerto, leida en retiro, el epilogo, elCesto y ALIADOS.
        // PROSA DE CLAUDE, marcada para revision (voz del cliente).
        { txt: "Pasarle la bola a Snotlig. Un domingo más, a tu derecha.", req: { rel: ["snotlig", 3], noflag: "cinturonASnotlig" }, forzable: true, tirada: { stat: "AG", obj: 9, riesgo: false,
          ok: { txt: "Se la pasas. Snotlig, cincuenta años y doce cascos, la coge y corre como corrió por el río el primer día: sin sentido y rápido, y ninguno de los Cuatro Dedos sabe qué es lo que se les escapa. Anota. Levanta la bola con las dos manos, se le cae, y la levanta otra vez. Esa noche se sienta bajo el puente, vivo, y se ríe mal. 'Ya', dice. Se va a dormir a tu derecha, un domingo más.", fx: { fama: 12, Honor: 2, gol: 1, rel: { snotlig: 5, banda: 3, aficion: 3 }, flag: "snotligUnDomingoMas" } },
          ko: { txt: "Se la pasas. Snotlig la coge, y el cuerpo, que tiene cincuenta años, va un paso por detrás de la cabeza. Un Cuatro Dedos joven le entra de frente, en la pata de atrás, como Snotlig te enseñó a entrar a ti. Cae. No se levanta. Se queda en el barro con la bola en las manos y los doce cascos puestos, a tu derecha, el domingo que él quiso jugar. No es una muerte triste: un goblin no llega a cincuenta años, y el que llega no tiene derecho a elegir cómo se va. El tuyo eligió. Colgáis su casco en el puente esa noche, y el tuyo al lado.", fx: { Honor: 2, Voluntad: 1, rel: { banda: 3, aficion: 2 }, flag: "snotligMuerto" } } } },
    ...
    elCesto: { titulo: "El cesto",
      texto: (pj) => `Vas al río, al sitio de la raíz. ${pj.flags.cascosEnElPuente ? "Los doce cascos de Snotlig cuelgan del puente." : ""} Hay un cesto enganchado. Dentro, una cría de orco, pequeña, de tres días, que te mira y te muerde el dedo. La charca sigue tirando a los pequeños. Nadie ha cambiado eso. Tú tampoco. ${pj.flags.teUnisteALaCharca ? "Eres de la charca ahora, comes primero, y la charca sigue tirando crías al río. Desde arriba se ve pequeño el cesto. Muy pequeño." : pj.flags.jefeSupremo ? "Eres el Jefe Supremo y la charca sigue tirándolos. Podrías haberlo cambiado. No lo hiciste. Es lo que hacen las charcas, y lo que hacen los jefes." : ""}`,
      opciones: [
        { txt: "Sacarla del cesto. Como te sacaron.", fx: { Honor: 3, rel: { banda: 3, snotlig: 2 }, flag: "sacasteALaCria" }, msg: (pj) => pj.flags.snotligMuerto ? "La sacas. Muerde. Se la das a Zig, o al primer goblin que pasa, porque el que la habría cogido ya no está. 'Muerde', dices. Es lo que dijeron de ti. Crecerá con lo que hay." : "La sacas. Muerde. Se la das a Snotlig, o a Zig, o al primer goblin que pasa. 'Muerde', dices. Es lo que dijeron de ti. Crecerá con lo que hay." },
    ...
    retiro: { titulo: "Lo que dura una banda",
      texto: (pj) => `Un orco deja de crecer cuando deja de pelear. ${pj.flags.teUnisteALaCharca ? "El Cáliz de Barro está en Gorgomor, en tu vitrina de la charca, y sabe raro cada vez que lo miras." : pj.flags.campeon ? "El Cáliz de Barro está bajo el puente, lleno de dientes, donde nadie lo busca." : "El Cáliz se quedó en el elefante, y da igual: Gorgomor se agacha igual, a medias."} ${pj.flags.snotligMuerto ? "Snotlig no llegó a ver esto: cayó en el barro un domingo, a tu derecha, con los doce cascos puestos. El sitio a tu derecha lo dejas vacío." : pj.rel.snotlig >= 4 ? "Snotlig duerme a tu derecha." : ""} ${pj.flags.grimgutzSeQueda || pj.flags.puenteConGrimgutz ? "Grimgutz mira la mosca." : ""} Queda decidir qué haces con lo que queda de ti, y con la banda.`,
      opciones: [
        { txt: "Ser Jefe Supremo hasta que te coman. Es lo que se hace.", req: { flag: "tumbasteAKrug", eje: { jefeIgual: 2 } }, fx: { Ferocidad: 2, rel: { aficion: 3, banda: 2 }, flag: "finJefe" }, msg: "Mandas Gorgomor desde un elefante muerto, con un goblin a la derecha, hasta que uno más grande te tira. Te comen. Es lo que se hace. Pero durante veinte años Gorgomor tuvo un jefe que contaba goblins, y eso no se lo comen." },
        { txt: "Volver al puente. Que la banda la mande Snotlig, o quien sea. Tú, a mirar el río.", fx: { Honor: 3, rel: { grimgutz: 5, snotlig: 3, banda: 2 }, flag: "finPuente" }, msg: (pj) => pj.flags.grimgutzVendido ? "Vuelves al puente, al hueco frío donde dormía un troll que vendiste. Nadie te hace sitio ya. Miras el río tú solo. La banda juega los domingos sin ti, y ganas o pierdes desde debajo del puente, oyéndolo, con un saco de oro que no abres porque huele a mosca." : "Vuelves al puente. Grimgutz te hace sitio. Miráis el río. La banda juega los domingos sin ti y ganas o pierdes desde debajo del puente, oyéndolo. Un día llega un árbitro por el camino y Grimgutz te mira. Le dices que no. Se lo come igual, por costumbre." },
        { txt: "Montar un circo de goblins. Que la Cristalvisión pague por verlos girar.", req: { eje: { deudaLibre: 2 } }, fx: { Astucia: 2, oro: 200, rel: { aficion: 2, banda: -1, skabnik: 3 }, flag: "finCirco" }, msg: "Montas el circo. Skabnik, si sigue, es la estrella: gira. Los goblins roban al público mientras mira. Ma Gorka lleva las cuentas. Es lo más rentable que ha hecho un orco, y lo menos orco." },
        { txt: "Un partido más. De frente. Contra quien sea.", req: { eje: { enteraSolo: 2 } }, fx: { Ferocidad: 1, fama: 10, rel: { banda: 2, aficion: 3 }, flag: "finBarro" }, msg: "Juegas uno más. Y otro. Con el cuerpo diciendo que ya. Un domingo vas de frente contra alguien más joven y no te levantas, y cien goblins se agachan, y Grimgutz viene y se sienta al lado, y te comen, como se hace, pero despacio." },
      ] },
  },
  epilogo: (pj, rasgo) => {
    ...
    return `${pj.nombre} fue ${rasgo}, y fue el pequeño de la camada, y le tiraron al río. ${pj.flags.teUnisteALaCharca ? "Ganó el Cáliz de Barro subido al elefante de Rey Krug, del lado de la charca que lo tiró, y desde arriba no volvió a ver a su banda." : pj.flags.campeon ? "Ganó el Cáliz de Barro con seis goblins, un troll de río y un chamán que fallaba." : "No ganó el Cáliz, y Gorgomor se agachó igual, a medias."} ${pj.flags.jefeSupremo ? "Fue Jefe Supremo de Gorgomor." : ""} ${pj.flags.snotligMuerto ? "Snotlig, que le sacó del cesto, cayó en el barro a su derecha el domingo que quiso jugar, con cincuenta años y doce cascos. Un goblin no llega a cincuenta. El suyo llegó." : pj.rel.snotlig >= 4 ? "Snotlig, que le sacó del cesto, durmió a su derecha hasta el final, que en un goblin es un milagro." : pj.rel.snotlig <= 0 ? "Snotlig se fue con sus doce cascos y no volvió." : ""} ${pj.flags.grimgutzSeQueda || pj.flags.puenteConGrimgutz ? "Grimgutz sigue bajo el puente, mirando la misma mosca." : ""} ${pj.flags.sacasteALaCria || pj.flags.cambiasteLaCharca ? "Hay al menos una cría que no sabe lo que es un cesto." : ""} ${pj.muertes > 0 ? `Murió ${pj.muertes + 1} veces; solo la última contó.` : ""} ${fin}`;
  },
  recuerdos: {
    ...
    cascosGanados: "Los cascos de los Charcos, ganados, no robados.", cascosRobados: "Snotlig los robó igual.", rugiste: "Un ruido que era tuyo.", mirasteAKrug: "Krug come primero y pega segundo. Se lo apuntaste en el estómago.", contasteDesdeElCesto: "Contaste desde el cesto. 'Este cuenta', dijo Snotlig.", teUnisteALaCharca: "Ganaste el Cáliz subido al elefante de Krug, del lado de la charca que te tiró.", tumbasteAKrug: "Tumbaste a Rey Krug, cuatro metros, en la pata de atrás.", snotligSeAgacho: "Se agachó. Un día se lo cobra.",
    ...
    snotligUnDomingoMas: "Un domingo más, a tu derecha. Corrió como el primer día.", snotligMuerto: "Cayó en el barro un domingo. Cincuenta años, doce cascos. Eligió jugar.",
...
const ORCO_ALIADOS = (pj, cap) => [
  { nombre: "Snotlig", ST: 1, AG: 4, AV: 7, si: pj.rel.snotlig >= 0 && !pj.flags.snotligMuerto && !(cap === 7 && pj.flags.cinturonASnotlig) },
...
const ORCO_ENTREACTOS = [
  ...
  { id: "jabalies", caps: [2, 3, 4], txt: "Cazar jabalíes a mano con los goblins. En la pata de atrás.", fx: { stat: { ST: 1 }, rel: { banda: 1 }, flag: "laPataDeAtras" }, msg: "Los jabalíes caen si les das en la pata de atrás. Todo cae si le das en la pata de atrás. Lo apuntas en el estómago." },
...
const HALFLING_ALIADOS = (pj, cap) => [
  { nombre: "Roblerto (el árbol)", ST: 6, AG: 1, AV: 11, si: !pj.flags.roblertoVendido && !pj.flags.fichastePorMortaigne },
...
const HALFLING = {
  ...
  capitulos: [
    ...
    { id: 2, titulo: "La liga de los mataos", sub: "Sexta División, o lo que quede debajo", escenas: ["debut", "derbiPringados", "roblertoYElArbitro", "palizaSemanal"] },
    { id: 3, titulo: "El escándalo del árbol", sub: "Cuando Roblerto se come lo que no debe", escenas: ["federacion", "quehacerConRoblerto", "sinArbol", "pipoAprovecha"] },
    { id: 4, titulo: "La oferta", sub: "Alguien grande te quiere. Para algo.", escenas: ["ojeador", "laDecision", "elGranClub"] },
    { id: 5, titulo: "La Copa de los Pringados", sub: "El torneo de los que no ganan nada nunca", escenas: ["copa", "semifinal", "laBorrachera", "finalCopa", "mortaigneLlegada", "mortaigneTercera", "laOlla", "mortaigneVuelta"] },
    { id: 6, titulo: "La final imposible", sub: "Contra los grandes, en el barro de verdad", escenas: ["visperas", "elCristal", "granFinal"] },
    { id: 7, titulo: "El ocaso del pastel", sub: "Lo que dura un halfling que sobrevivió", escenas: ["despues", "laAbuela", "laCena", "ultimoPartido", "retiro"] },
  ],
  escenas: {
    cocina: {
    ...
    pipo: {
      ...
      opciones: [
        ...
        { txt: "Regatearle a base de tartas.", fx: { rel: { pipo: 1 }, Astucia: 1, flag: "negociastePipo" }, msg: "Acabáis en un cinco por ciento y dos tartas al mes. Es la mejor negociación de tu vida y la peor de la suya. Pipo firma con una sonrisa que no le llega a los ojos: nunca olvidará aquel cinco por ciento, y nunca lo perdonará del todo. A partir de ahí sois amigos, que en Pipo significa que te robará más despacio y con más rencor." },
      ],
    },
    arbol: {
      titulo: "El fichaje estrella",
      texto: (pj) => `Un equipo de halflings no gana partidos: gana tiempo hasta que se despierta el árbol. El de los Comepasteles vive en la charca de detrás del horno y responde, a veces, al nombre de Roblerto. Mide lo que tres carros, tiene la inteligencia de un banco de jardín y una manía preocupante: coge del suelo lo pequeño y redondo y lo lanza muy lejos. En un campo, eso es el balón. Fuera del campo, eso eres tú.`,
      opciones: [
        { txt: "Hacerte amigo del árbol. A base de pasteles.", fx: { rel: { arbol: 2 }, Honor: 1, flag: "amigoArbol" }, msg: "Le llevas pastel cada día. Roblerto aprende a distinguirte del balón por el olor a mantequilla. Es lo más parecido a un seguro de vida que vas a tener." },
        { txt: "Comerte tú el pastel que era para Roblerto.", fx: { Voluntad: 1, rel: { arbol: -1 }, flag: "teComisteSuPastel" }, msg: "Está buenísimo. Roblerto se entera, porque los árboles se enteran de todo despacio. A partir de hoy te mira como se mira a un balón que encima te ha robado la merienda." },
        { txt: "Mantenerte muy lejos de Roblerto.", fx: { Astucia: 1, Voluntad: 1, flag: "lejosArbol" }, msg: "Decides que el árbol es cosa de los demás. Es la decisión más sensata que tomarás en tu carrera, y por eso mismo no durará." },
      ],
    },
    chef: {
      titulo: "El Chef",
    ...
    primerPartido: {
      ...
      opciones: [
        ...
        { txt: "Que Roblerto te lance a la zona de anotación como una bomba de mantequilla.", req: { flag: "amigoArbol", noflag: "roblertoVendido" }, forzable: true, tirada: { stat: "ST", obj: 7, riesgo: true,
          ok: { txt: "Roblerto te agarra, apuntas al hueco, y sales disparado por encima de doce carniceros que miran hacia arriba con la boca abierta. Cruzas la línea volando y aterrizas de morros, pero cruzas. La jugada se hará famosa. La copiarán fatal en toda la Comarca.", fx: { fama: 10, Ferocidad: 1, gol: 1, rel: { arbol: 2, aficion: 2 }, flag: "labomba" } },
          ko: { txt: "Roblerto te agarra, se distrae con una mariposa, y te lanza a la grada. Caes de lleno sobre el puesto de tartas de tu abuela. Ella, sin inmutarse, te quita una pasa de la oreja: 'Te lo dije'.", fx: { Voluntad: 1, golRival: 1 } } } },
        { txt: "Sentarte encima del balón y aguantar el chaparrón.", tirada: { stat: "AG", obj: 7, riesgo: false,
          ok: { txt: "Te sientas encima del balón, te haces bola, y aguantas doce placajes que rebotan en ti como en un flan. El partido se atasca, nadie mueve el marcador, y aguantar así es, para un Comepasteles, una gesta que se cantará en las cocinas durante generaciones.", fx: { Voluntad: 2, rel: { equipo: 2, aficion: 1 }, flag: "teSentaste" } },
        ...
        { txt: "Olvidarte del balón y proteger a un compañero que está en el suelo.", tirada: { stat: "ST", obj: 8, riesgo: true,
          ok: { txt: "Te plantas delante del compañero mientras tres Carniceros se acercan. No puedes ganar. Tampoco puedes hacer nada. Pero cuando el primero llega, le muerdes. El segundo se ríe. El tercero deja de reírse. Aunque el equipo pierda, esa noche en el vestuario nadie vuelve a llamarte enclenque.", fx: { Honor: 1, Ferocidad: 1, Voluntad: 1, rel: { equipo: 3, aficion: 1 }, flag: "protegisteCaido" } },
          ko: { txt: "Te plantas delante del compañero, pero eres tres palmos y ellos tres carniceros. El primero te aparta de un manotazo y al compañero lo levantan igual, y a ti contigo. Lo intentaste, y en el vestuario lo vieron. No es ganar. Pero tampoco es nada.", fx: { Voluntad: 1, rel: { equipo: 1 }, flag: "protegisteCaido" } } } },
      ],
    },
    ...
    derbiPringados: {
      ...
      opciones: [
        ...
        { txt: "Que Roblerto despeje la merienda de un manotazo y correr tú.", req: { flag: "amigoArbol", noflag: "roblertoVendido" }, forzable: true, tirada: { stat: "ST", obj: 7, riesgo: true,
          ok: { txt: "Roblerto barre la mesa de la merienda con un brazo y Villapán entero se lanza a por los pasteles que vuelan. En el caos, cruzas la línea andando, silbando. La victoria más ridícula de la historia de Sexta, y es vuestra.", fx: { fama: 8, gol: 1, rel: { arbol: 1, equipo: 2 }, flag: "ganasteAlgo" } },
          ko: { txt: "Roblerto barre la mesa, sí, pero también te barre a ti, que estabas encima cogiendo un trozo. Vuelas con los pasteles. Aterrizas en el pastel envenenado de la defensa. Sabe raro.", fx: { golRival: 1 } } } },
        { txt: "Aburrirlos hasta que se coman entre ellos.", tirada: { stat: "AG", obj: 7, riesgo: false,
          ok: { txt: "No haces nada. Absolutamente nada, durante veinte minutos. Villapán, sin drama que comer, empieza a picar de su propia merienda y a discutir, y a empujarse, y a placarse entre ellos. Cruzas mientras se pelean y os lleváis el partido por incomparecencia moral. Épico.", fx: { fama: 5, Voluntad: 1, gol: 1, rel: { equipo: 1 }, flag: "ganasteAlgo" } },
    ...
    roblertoYElArbitro: {
      ...
      condicion: (pj) => !pj.flags.roblertoVendido,
      texto: (pj) => `Pasa lo que tenía que pasar. En pleno partido, el árbitro —un goblin con silbato— le saca tarjeta a Roblerto por "existir con intención de placar". Roblerto, que confunde lo pequeño y ruidoso con comida, se lo mete en la boca de un solo movimiento, con silbato y todo. Se hace un silencio que solo rompe el silbato, ahora dentro de Roblerto, pitando cada vez que respira. La federación abre expediente. ${pj.rel.arbol >= 3 ? "Roblerto te busca con la mirada, culpable, con media tarjeta amarilla asomando entre las ramas." : "Roblerto ni se inmuta. Sigue buscando el balón, o el postre, que para él es lo mismo."}`,
      opciones: [
        { txt: "Cubrir a Roblerto: 'se marchó a por tabaco, señoría'.", fx: { Honor: -1, rel: { arbol: 2 }, flag: "cubristeRoblerto" }, msg: "Juras que el árbitro se fue voluntariamente. Nadie te cree, pero nadie quiere acercarse a Roblerto a comprobarlo. El expediente se queda en un cajón. Roblerto, a su manera lentísima, lo entiende, y te trae una piedra de regalo." },
        { txt: "Enseñar a Roblerto a escupir árbitros. Solo escupir.", req: { rel: ["arbol", 2] }, forzable: true, fx: { Astucia: 1, rel: { arbol: 1 }, flag: "roblertoEscupe" }, msg: "Semanas de galletas y paciencia. Roblerto aprende a escupir lo que no es redondo. El árbitro sale mareado y sordo del silbato, pero vivo. Es más de lo que consigue la mayoría de la gente con un hombre-árbol suelto." },
        { txt: "Vender a Roblerto antes de que se coma a alguien importante.", fx: { oro: 100, Honor: -1, rel: { arbol: -3 }, flag: "roblertoVendido", flags: ["cedisteALaTentacion"] }, msg: "Un circo de Cuarta paga cien coronas por un árbol que se come árbitros. Roblerto se va sin mirar atrás, porque los árboles no miran atrás. El vestuario queda más seguro y muchísimo más triste. Ganaréis menos. Comeréis mejor. No es lo mismo." },
      ],
    },
    palizaSemanal: {
      titulo: "La paliza de los jueves",
      partido: { rival: "Los Rompehuesos de Grauwald", fuerza: 4 },
      texto: (pj) => `Toca lo de siempre: un equipo grande que os usa de entrenamiento. Los Rompehuesos de Grauwald tienen tres jugadores con nombre de arma de asedio y un plan de partido de una sola palabra. ${pj.flags.roblertoVendido ? "Sin Roblerto, la única táctica es correr y rezar, y los halflings no sois de rezar." : "Roblerto, al menos, iguala un poco la báscula, aunque hoy mira una nube en vez del campo."} ${pj.flags.prometisteSobrevivir ? "Recuerdas tu promesa: sobrevivir la temporada. Hoy toca cumplirla, no ganarla." : ""} {marcador}. La grada local ha traído carteles con el número de bajas que esperan. Van ganando ellos, los carteles.`,
      opciones: [
    ...
    quehacerConRoblerto: {
      ...
      condicion: (pj) => !pj.flags.roblertoVendido,
      texto: (pj) => `Roblerto es un problema con raíces. Se come lo que no debe, tira lo que no toca, y la mitad de las semanas juega mirando una mariposa. Pero cuando acierta —una vez cada seis partidos— gana él solo. ${pj.rel.arbol >= 4 ? "Y además es, a su manera de banco de jardín, tu amigo: te distingue del balón, te trae piedras, te vigila mientras duermes en el autobús." : "Y además es lo único que os separa de perder por cincuenta cada domingo."} El equipo espera que decidas qué clase de árbol vais a ser.`,
      opciones: [
        { txt: "Entrenarlo en serio. Con pasteles y mucha paciencia.", fx: { rel: { arbol: 2, equipo: 1 }, Voluntad: 1, flag: "roblertoEntrenado" }, msg: "Meses de galletas y señales. Roblerto aprende a distinguir el balón del árbitro, casi. Pasa de acertar una de seis a una de cuatro, que en un hombre-árbol es un doctorado. El equipo empieza a creer en algo por primera vez." },
        { txt: "Dejarlo a su aire. Un árbol es un árbol.", fx: { Honor: 1, rel: { arbol: 1 }, flag: "roblertoLibre" }, msg: "Decides que Roblerto es Roblerto y que quien quiera un árbol domesticado que se compre un bonsái. Sigue mirando mariposas y comiéndose lo que no debe, pero es feliz, y su felicidad, un día, cruzará la línea por accidente." },
        { txt: "Convertirlo en negocio: entradas para 'ver al árbol'.", req: { flag: "firmasteAPipo" }, forzable: true, fx: { oro: 60, fama: 6, Honor: -1, rel: { arbol: -1, pipo: 1 }, flag: "roblertoEspectaculo" }, msg: "Pipo monta la atracción: 'Venga a ver al árbol que se comió a un árbitro'. La grada se llena de morbosos con niños. Roblerto, que no entiende de qué va, posa. Ganáis dinero. Roblerto deja de traerte piedras: algo ha cambiado." },
      ],
    },
    sinArbol: {
      titulo: "El partido sin red",
      partido: { rival: "Los Cuervos Rotos de Mortaigne", fuerza: 3 },
      texto: (pj) => `${pj.flags.roblertoVendido ? "Sin Roblerto desde hace semanas, jugáis como lo que sois: once pasteles corriendo." : pj.flags.roblertoEntrenado ? "Roblerto está sancionado un partido por lo del árbitro, así que hoy jugáis sin red, con él en la grada comiendo palomitas y mirándoos preocupado." : "Roblerto hoy mira una nube y no piensa bajar, así que estáis solos."} Enfrente, un equipo de no-muertos de segunda mano —los Cuervos Rotos— a los que les faltan piezas y no les sobra piedad. ${pj.flags.aguantasteEntero ? "Al menos ya sabéis aguantar de pie. Eso hoy vale oro." : ""} {marcador}. Un partido para halflings de verdad: sin trucos, sin árbol, sin excusas.`,
      opciones: [
    ...
    pipoAprovecha: {
      ...
      texto: (pj) => `Pipo Cazuelas te espera con dos puros encendidos, uno para cada mano de tres dedos. "Chaval, tenemos algo", dice. "La gente no viene a veros ganar, que eso no pasa. Viene a veros sufrir con gracia. Eso, amigo mío, es un producto." ${pj.flags.negociastePipo ? "Y antes de nada, desliza una cláusula: 'Ese cinco por ciento tuyo… este año lo corregimos, ¿eh?'. No ha olvidado aquella negociación. Un halfling no olvida un cinco por ciento que se le escapó, y este viene a recuperarlo." : pj.flags.leisteContrato ? "Esta vez te trae el contrato con las cláusulas turbias ya tachadas por él mismo: 'Para ahorrarte el placer, listo'. Desde que le leíste las catorce páginas, no se fía de ti, y en Pipo eso es casi cariño." : pj.flags.firmasteAPipo ? "Te presenta a un socio como 'mi jugador más rentable, el que firma sin leer', y por una vez no está mintiendo: llevas años firmándole todo." : ""} ${pj.flags.roblertoEspectaculo ? "Ya ha triplicado la entrada con lo del árbol. Ahora quiere más." : "Tiene un plan, y los planes de Pipo siempre acaban con él más rico y contigo más famoso, o más muerto."} Sobre la mesa, un contrato nuevo. Catorce páginas, otra vez. La letra pequeña ha crecido.`,
      opciones: [
    ...
    ojeador: {
      ...
      opciones: [
        ...
        { txt: (pj) => `Preguntarle si tu equipo, ${pj.flags.roblertoVendido ? "Bortrand incluido" : "Roblerto y Bortrand incluidos"}, viene contigo.`, fx: { Honor: 2, rel: { equipo: 2 }, flag: "preguntastePorLosTuyos" }, msg: (pj) => `El ojeador se ríe sin ganas. '${pj.flags.roblertoVendido ? "No compro cocineros" : "No compro leña ni cocineros"}. Te compro a ti'. Le dices que lo pensarás. Los dos sabéis que preguntar eso ya es media respuesta. En el vestuario, cuando lo cuentas, Bortrand te sirve doble ración sin decir nada.` },
    ...
    elGranClub: {
      ...
      opciones: [
        ...
        { txt: "Buscar al compañero mejor colocado y dársela.", tirada: { stat: "AG", obj: 9, riesgo: false,
          ok: { txt: "Sueltas un pase que cruza el barro y cae en manos amigas. Cruza. El pase de un halfling que ve el campo desde abajo mejor que nadie: nadie mira nunca a la altura de un halfling.", fx: { fama: 7, Astucia: 1, gol: 1, pase: 1 } },
    ...
    copa: {
      ...
      condicion: (pj) => !pj.flags.fichastePorMortaigne,
      ...
      opciones: [
        { txt: "Tomártelo en serio. Es la primera copa que podéis ganar.", fx: { Ambición: 1, Voluntad: 1, rel: { equipo: 1 }, flag: "enSerioLaCopa" }, msg: "Entrenáis. De verdad. Roblerto mira menos mariposas, Bortrand cocina para dar fuerza y no solo sabor, y tú aprendes las jugadas al derecho y al revés. Un equipo de pringados tomándose algo en serio da hasta miedo." },
    ...
    finalCopa: {
      ...
      opciones: [
        { txt: "La jugada de la piña, a lo grande, con todo el equipo.", req: { flag: "jugadaPiña" }, forzable: true, tirada: { stat: "AG", obj: 8, riesgo: true,
          ok: { txt: (pj) => `Los once os hacéis albóndiga y rodáis hacia la línea con el balón dentro${pj.flags.roblertoVendido ? ", empujándoos unos a otros porque ya no hay árbol que empuje" : " y a Roblerto empujando por detrás como una apisonadora vegetal"}. Los Segadores siegan el aire. La piña cruza. Habéis ganado la Copa de los Pringados. El latón nunca ha brillado tanto.`, fx: { fama: 15, Ambición: 1, gol: 1, rel: { equipo: 3, aficion: 3 }, flag: "campeon", flags: ["ganasteLaCopa"] } },
    ...
    /* ---- CAP 5 · RAMA MORTAIGNE (solo si fichastePorMortaigne) ----
       Prosa de Claude, comedia negra más oscura. MARCADA PARA REVISIÓN.
       Reconverge en el cap 6 (visperas) con los Comepasteles. */
    mortaigneLlegada: {
      titulo: "La camiseta negra",
      condicion: (pj) => !!pj.flags.fichastePorMortaigne,
      texto: (pj) => `Mortaigne no huele a estofado. Huele a formol, a linimento caro y a un vestuario donde nadie te da los buenos días, porque los buenos días no ganan partidos. El míster, Serrault, un humano flaco con voz de contable, te tira una camiseta negra a la cara: "No te quiero simpático. Te quiero ganando. Lo primero lo has sido siempre; lo segundo no lo has sido en tu vida." Drache y Kessler, los dos veteranos, te miran como se mira un fichaje de marketing: una cosa pequeña y ruidosa que vende cromos. Aquí nadie te confundirá con un balón. Aquí saben exactamente lo que eres.`,
      opciones: [
        { txt: "Ponerte la negra y callarte. Aprender a ganar aunque duela.", fx: { Ambición: 2, Astucia: 1, fama: 6, rel: { abuela: -1 }, flag: "camisetaNegraPuesta" }, msg: "Te la pones. Pesa más que la de los Comepasteles, y no por la tela. Esa noche cenas solo un plato caliente que no te ha guardado nadie, y aprendes la primera lección de Mortaigne: aquí se gana, y ganar se paga por adelantado, en soledad." },
        { txt: "Ponértela, pero con la de los Comepasteles debajo, donde no la vean.", fx: { Honor: 1, Astucia: 1, Voluntad: 1, flag: "camisetaEscondida" }, msg: "Debajo del negro, la camiseta de los pasteles, sudada y con una mancha de estofado que no se fue. Nadie lo sabe. Es la primera cosa tuya que escondes, y no será la última; pero mientras la lleves debajo, no eres del todo suyo." },
        { txt: "Preguntar por 'la olla', de la que todos hablan a medias.", req: { Astucia: 3 }, forzable: true, fx: { Astucia: 1, flag: "oisteLoDeLaOlla" }, msg: "Kessler sonríe sin ganas. 'La olla es lo que nos hace ganar cuando el fútbol no basta', dice, y no dice más. Aprendes que en Mortaigne hay cosas que se cocinan y no se comen, y que un día te ofrecerán el cazo." },
      ],
    },
    mortaigneTercera: {
      titulo: "Aprender a hacer daño",
      condicion: (pj) => !!pj.flags.fichastePorMortaigne,
      partido: { rival: "Los Toros Rojos de Norburgo", fuerza: 3 },
      texto: (pj) => `Tu primer partido de Tercera de verdad. Los Toros Rojos corren, pegan y no se ríen de tu tamaño: te estudian, que es peor. Serrault, en la banda, no te pide que marques. Te pide otra cosa: "Su corredor es un chaval de veinte que vale tres de ti. Quítalo de en medio. No te pago para hacer gracia." Drache te ha enseñado dónde poner el hombro para que el árbitro no lo vea. {marcador}`,
      opciones: [
        { txt: "Hacer lo que te piden: quitar de en medio a su corredor.", tirada: { stat: "ST", obj: 9, riesgo: true,
          ok: { txt: "Esperas el bloqueo, pones el hombro donde Drache te enseñó, y el chaval de veinte se dobla en el barro sujetándose la rodilla. El árbitro mira a otro lado, que es su especialidad. Ganáis por su ausencia. Serrault asiente por primera vez, y te sube algo por dentro que no sabes si es orgullo o vergüenza.", fx: { fama: 8, Ferocidad: 2, baja: 1, Honor: -2, flag: "hicisteDanoEnMortaigne" } },
          ko: { txt: "Vas a por la rodilla del chaval y fallas: es más rápido y te deja mordiendo barro. Encima el árbitro, por una vez, mira. Amarilla. Serrault niega con la cabeza, despacio: en Mortaigne, fallar la jugada sucia es peor que no hacerla.", fx: { golRival: 1 } } } },
        { txt: "Jugar limpio, a tu manera. Que aprendan que un pastel corre.", tirada: { stat: "AG", obj: 8, riesgo: true,
          ok: { txt: "Pasas del encargo y juegas a lo tuyo: esquivas al corredor en vez de romperlo, y le robas la bola por abajo, donde nadie mira. Cruzas. Ganáis igual, pero limpio. Serrault no aplaude —'Bonito. Inútil, pero bonito'—, pero Kessler te mira distinto. Quizá no todo Mortaigne es Mortaigne.", fx: { fama: 6, gol: 1, Honor: 1, flag: "limpioEnMortaigne" } },
          ko: { txt: "Intentas el regate de pastel contra gente que corre de verdad, y te comen. Sin gol y sin excusa: aquí no vale el 'pero lo intenté con gracia'. Serrault apunta algo en su libreta que no quieres leer.", fx: { golRival: 1 } } } },
      ],
    },
    laOlla: {
      titulo: "La olla",
      condicion: (pj) => !!pj.flags.fichastePorMortaigne,
      texto: (pj) => `La noche antes del partido que decide el ascenso, Kessler te lleva a la cocina del estadio, que a estas horas no cocina nada de cena. Sobre el fuego, una olla que no huele a comida. 'El estofado del rival lo trae su utillero por esa puerta', dice. 'Un cazo de esto y mañana juegan doblados. Nadie lo prueba, nadie lo ve, todos lo saben y nadie lo dice. Así se gana en Mortaigne.' Te tiende el cazo. Pesa como pesó la camiseta negra, y más. ${pj.rel.abuela >= 3 ? "Piensas en tu abuela, que envenena a la gente al revés: llenándola de comida hasta que te quiere." : "Piensas en tu abuela, en su cocina, donde una olla solo servía para dar de comer."}`,
      opciones: [
        { txt: "Coger el cazo. Ganar es ganar, y estás aquí para eso.", fx: { Ambición: 1, Ferocidad: 1, Honor: -3, fama: 5, flag: "envenenasteConMortaigne", flags: ["cedisteALaTentacion"] }, msg: "Echas el cazo en su olla. No te tiembla la mano, y eso es lo que peor llevas. Al día siguiente el rival juega verde y doblado, ascendéis, y tu nombre sale en la crónica. Esa noche, por primera vez, no sueñas con tarta. No sueñas con nada." },
        { txt: "Devolverle el cazo lleno. No has llegado tan lejos para esto.", fx: { Honor: 3, Voluntad: 2, rel: { abuela: 1 }, flag: "teNegasteALaOlla" }, msg: "Le devuelves el cazo sin tocar. Kessler no se enfada; casi parece aliviado. 'Yo tampoco pude, la primera vez', dice, y tira el contenido al desagüe. Al día siguiente sudáis el ascenso limpio, o no lo sudáis; pero cenas, y duermes, que en Mortaigne es más de lo que tienen casi todos." },
        { txt: "Volcar la olla y contarle a Serrault lo que se cuece.", req: { Ferocidad: 4 }, forzable: true, fx: { Honor: 2, Ferocidad: 1, fama: 4, rel: { abuela: 1 }, flag: "volcasteLaOlla" }, msg: "Vuelcas la olla de una patada de tres palmos y vas derecho a Serrault. Él te escucha, mira a Kessler, y no hace nada: en Mortaigne la olla es de la casa. Desde esa noche te dejan solo con tu camiseta negra y tu conciencia limpia, que aquí es la peor compañía. Te has hecho un enemigo con nombre." },
      ],
    },
    mortaigneVuelta: {
      titulo: "Lo que no se compra",
      condicion: (pj) => !!pj.flags.fichastePorMortaigne,
      texto: (pj) => `${pj.flags.envenenasteConMortaigne ? "Ascendisteis. Tienes dinero, tienes nombre, y tienes un sabor en la boca que ningún vino quita." : "Ascendáis o no, ya da igual: has aprendido lo que se aprende aquí, y era más de lo que querías saber."} Una tarde, en el mercado de Mortaigne, hueles estofado en un puesto y se te doblan las rodillas sin permiso. Villapastel queda a tres días de carro y a toda una vida. Los Comepasteles siguen ahí, perdiendo con gracia, y corre el rumor de que la Cristalvisión monta una final de morbo contra los Carniceros y busca un gancho. ${pj.rel.abuela >= 3 ? "Tu abuela, dicen, pregunta por ti cada domingo y pone tu plato por si acaso." : "Nadie en Villapastel pregunta por ti, y eso, ahora, es lo que más pesa."}`,
      opciones: [
        { txt: "Guardar la camiseta negra en el fondo de la bolsa y volver al barro.", fx: { Honor: 2, Voluntad: 1, rel: { abuela: 1, equipo: 2, aficion: 1 }, flag: "volvisteDeMortaigne", flags: ["dejasteMortaigne"] }, msg: "Metes la camiseta negra en el fondo de una bolsa y coges el carro de vuelta. Los Comepasteles te reciben sin preguntar, porque los halflings no preguntan de dónde vuelve uno, solo si trae hambre. Roblerto, si sigue, te reconoce por el olor. Estás en casa, y hay una final imposible esperándote." },
        { txt: "Volver, pero con lo aprendido. Ahora sabes ganar, y no lo olvidas.", fx: { Ambición: 1, Astucia: 1, rel: { equipo: 1 }, flag: "volvisteDeMortaigne", flags: ["volvisteConColmillo"] }, msg: "Vuelves a los Comepasteles, sí, pero traes Mortaigne pegado a la bota: sabes dónde poner el hombro, sabes esperar, sabes que ganar a veces es feo. Bortrand lo nota en cómo miras al rival. 'Has vuelto distinto', dice, sirviéndote doble. Distinto no es peor. Todavía no." },
      ],
    },
    /* ---- CAP 6: La final imposible ---- */
    visperas: {
      titulo: "Lo que nadie esperaba",
      texto: (pj) => `${pj.flags.fichastePorMortaigne ? "Volviste de Mortaigne con nombre de Tercera y la camiseta negra en el fondo de un cajón. Cuando la Cristalvisión monta la final de morbo, tú eres el gancho perfecto: el pastel que jugó con los grandes, aprendió a hacer daño y volvió al barro. Los Comepasteles te acogieron sin preguntar, y hoy saltan contigo." : pj.flags.ganasteLaCopa ? "Ganar la Copa de los Pringados os ha convertido en una historia. Y las historias, en este deporte, suben. Os han invitado —por morbo, por marketing, por lo que sea— a jugar la final de un torneo de verdad contra un grande." : "No habéis ganado nada, pero habéis perdido con tanta gracia toda la temporada que la Cristalvisión os ha invitado a la final de un torneo de verdad 'para dar color'. Sois el color."} Contra los Carniceros de Bögenhafen, los mismos que en tu primer domingo vinieron a Villapastel a hacer manos. Han crecido. Tú también, aunque midas lo mismo. ${pj.flags.protegisteCaido ? "En el vestuario nadie discute quién habla: desde aquel primer domingo en que te plantaste delante de un compañero caído y le mordiste la bota a un carnicero, este equipo te sigue a donde haga falta." : pj.rel.equipo >= 5 ? "El vestuario, que empezó riéndose de ti, hoy se calla cuando hablas." : ""} ${pj.rel.abuela >= 3 ? "Tu abuela ha sacado el mantel bueno, el que solo sale en las bodas y los entierros." : ""}`,
      opciones: [
    ...
    despues: {
      ...
      opciones: [
        { txt: "Enseñar a los pequeños del pueblo a jugar. Y a caer.", fx: { Honor: 2, rel: { equipo: 1, aficion: 1 }, flag: "enseñaste" }, msg: "Montas una escuela en el campo de tablones. Les enseñas a esquivar, a esperar, a hacer la piña, y sobre todo a caer sin romperse. La mitad quiere ser tú. La otra mitad quiere ser Roblerto. Ninguno quiere ser carnicero. Algo has hecho bien." },
    ...
    laCena: {
      titulo: "La cena que no hubo que guardar",
      texto: (pj) => {
        const estuviste = !pj.flags.fichastePorMortaigne && pj.rel.abuela >= 3 && (pj.flags.ultimaComida || pj.flags.leContasteALaAbuela);
        const abrir = `No hay partido esta semana. No hay entrenamiento. Hay un carro que viene de Villapastel con un halfling en el pescante que no te mira a los ojos cuando te da la noticia, porque no hace falta. Lo sabes antes de que abra la boca. Lo supiste, en realidad, la última vez que la viste, aunque entonces preferiste no saberlo. Tu abuela ha muerto como vivió: sin avisar, sin drama, con el guiso a medias.`;
        const cuerpo = estuviste
          ? `Llegas a tiempo de lo que importa, que no es el final, sino todo lo de antes. Estuviste. ${pj.flags.ultimaComida ? "Comisteis en silencio la última vez, que entre vosotros era la manera de decirlo todo. " : ""}${pj.flags.leContasteALaAbuela ? "Le contaste lo del carnicero y ella tejió sonriendo. " : ""}Cerró los ojos sabiendo exactamente quién eras: el único tonto de la Comarca que se subió a un carro. En la cocina, la labor sigue caída donde la dejó. Y en la ventana, la única que da al camino, ya no hay nadie mirando pasar los carros. Pero durante un tiempo lo hubo, y te miraba a ti.`
          : `Te enteras tarde. Siempre te enteras tarde de lo que pasa en Villapastel, porque Villapastel queda lejos de donde tú decidiste estar. ${pj.flags.fichastePorMortaigne ? "Llevas la camiseta negra puesta cuando te lo dicen, y por primera vez te pesa como lo que es: una tela que te llevó lejos de la única ventana que importaba. " : ""}Te guardó la cena cada noche. Muerto o vivo, decía, la cena a las ocho. La guardó todas las noches que estuviste fuera, que fueron muchas, hasta que una noche no hubo que guardarla.${pj.flags.mentiraPiadosa ? " Lo último que le dijiste fue que lo dejabas, que volvías a la cocina, que te quedabas. Fue mentira, y ella lo sabía, y aun así te guardó la cena. Las abuelas saben cuándo les mientes. Te dejan hacerlo porque a veces la mentira es la única tarta que le queda a uno para ofrecer." : ""}`;
        const cierre = `Vuelves a Villapastel para el entierro. Es pequeño, como todo en la Comarca, como tú. Y de vuelta, en el carro, te das cuenta de que llevas toda la vida corriendo hacia el camino de esa ventana, y que ella se quedó en el otro lado, viéndote ir, tejiendo cada vez más despacio. No dejes que te vean pequeño, te dijo una vez. Y si te ven, muérdeles el tobillo. Nunca te dijo qué hacer cuando el tobillo que te falta por morder es el de la muerte, que no tiene tobillos y no se ríe de tu tamaño porque le das exactamente igual. Así que haces lo único que sabes hacer.`;
        return `${abrir} ${cuerpo} ${cierre}`;
      },
      opciones: [
        { txt: "Amasar. Como el primer día.", fx: { Voluntad: 2, Honor: 1, flag: "enterrasteALaAbuela" }, msg: "Amasas. Un día entero, en su cocina, con su receta. Y cuando sacas la tarta del horno, la dejas enfriándose en la ventana. Por si acaso. Por si algún carro pasa." },
      ],
    },
    ultimoPartido: {
      titulo: "El último domingo",
      partido: { rival: "Los Tragaldabas de Villapán", fuerza: 2 },
      texto: (pj) => `Tu último partido, y el destino, que tiene sentido del humor halfling, te enfrenta a los Tragaldabas de Villapán, los mismos del derbi de los descartes de aquella primera temporada, ahora con nietos gordos en el equipo. Es un partido que no decide nada: ni copa, ni liga, ni gloria. Solo tú, el barro, y las rodillas diciéndote que es la última vez. ${pj.flags.enseñaste ? "En la grada, media escuela de pequeños que enseñaste mira, aprendiendo cómo se dice adiós." : ""} ${pj.flags.enterrasteALaAbuela ? "Tu abuela ya no está para verlo, pero llevas en el bolsillo un trozo de la última tarta, envuelto en su servilleta." : pj.rel.abuela >= 4 ? "Tu abuela ha venido, con el mantel bueno y la labor, a ver el último." : ""} {marcador}. No hay nada en juego. Por eso importa tanto.`,
      opciones: [
    ...
    retiro: {
      ...
      texto: (pj) => `Se acabó. Cuelgas unas botas que nunca te quedaron bien del todo en el clavo del vestuario que huele a estofado. ${pj.flags.ganasteEnGrande ? "Eres una leyenda: el halfling que ganó una final de verdad. Los pequeños de toda la Comarca juegan a ser tú, y pierden, y no les importa." : pj.flags.ganasteLaCopa ? "Tienes una copa de latón abollado en la repisa del horno, y es más que muchos grandes con vitrinas enteras." : "No ganaste casi nada, pero sobreviviste a todo, que en un halfling es la mayor de las hazañas."} ${pj.flags.fichastePorMortaigne ? "La camiseta negra de Mortaigne sigue en un cajón. Un día la quemas en el horno. Huele a formol. El pan de esa hornada nadie lo compra." : ""} Roblerto, si sigue, mira una mariposa. Bortrand cocina. ${pj.flags.negociastePipo ? "Pipo cuenta, por enésima vez, el cinco por ciento que nunca le dejaste subir: sigue enfadado, sigue viniendo, y en el fondo es lo más parecido a lealtad que tiene." : pj.flags.leisteContrato ? "Pipo cuenta un dinero limpio, por una vez, porque tú leíste hasta la última página y no le quedó letra pequeña donde esconderse." : "Pipo cuenta un dinero que ya no os debe, y aún le parece poco."} Y tú, por fin, ${pj.flags.enterrasteALaAbuela ? "te sientas a la mesa vacía de tu abuela, sin prisa, con dos platos puestos por costumbre." : "te sientas a la mesa de tu abuela sin prisa."}`,
      opciones: [
  ...
  muertes: [
    ...
    { titulo: "Roblerto", texto: "Roblerto te recoge del barro creyéndote el balón, te aprieta para lanzarte, y del susto el corazón te arranca a latir de nuevo. Te suelta, decepcionado de que no seas redondo. Le debes la vida y un pastel. Desde entonces andas un pelín más lento.", fx: { stat: { MA: -1 }, Voluntad: 1 } },
  ...
  epilogo: (pj, rasgo) => `${pj.nombre} fue ${rasgo}, midió tres palmos y sobrevivió a bastante más de lo que sobrevive un pastel en aquella cocina. ${pj.flags.teSentaste ? "Inventó la táctica de sentarse encima del balón, que hoy enseñan hasta los orcos." : pj.flags.labomba ? "Su touchdown volando por encima de doce carniceros se copió fatal en toda la Comarca durante años." : ""} ${pj.rel.abuela >= 3 ? "Su abuela le guardó la cena cada noche, muerto o vivo, hasta que una noche no hubo que guardarla." : "Su abuela dejó de poner su plato el día que se fue, y no lo volvió a poner."} ${pj.muertes > 0 ? `Murió ${pj.muertes + 1} veces, todas ridículas, y solo la última contó.` : ""} ${pj.flags.prometisteGanar ? ((pj.palmares || []).filter((p) => p.res === "Victoria").length ? `Cumplió la promesa que nadie le creyó en aquel vestuario: ganó, cuando el pueblo entero había apostado la fecha en contra.` : `Nunca cumplió la promesa de ganar un partido —ni uno—, pero la hizo riéndose, así que a nadie defraudó.`) : ""} Que un halfling durase tanto fue, según Pipo Cazuelas, ${pj.flags.negociastePipo ? `"el mejor negocio que hice; y perdí dinero, por aquel maldito cinco por ciento que nunca le pude subir".` : pj.flags.firmasteAPipo ? `"el mejor negocio que hice, y no pienso decir cuánto gané: firmaba sin leer, bendito sea".` : `"el mejor negocio que hice; y ni siquiera pude engañarle, que es lo que más me duele".`}`,
  ...
  recuerdos: { conOdio: "Amasabas el pan con odio.", firmasteAPipo: "Le debes tartas a Pipo de por vida.", negociastePipo: "Le ganaste un cinco por ciento a Pipo, y no te lo perdonó jamás.", leisteContrato: "Leíste las catorce páginas y le tachaste la cláusula de las tartas.", amigoArbol: "Roblerto te distinguía del balón por el olor a mantequilla.", teSentaste: "Te sentaste encima del balón y sobreviviste.", labomba: "Volaste por encima de doce carniceros.", anotasteElPrimero: "Anotaste el primer touchdown halfling que se recuerda en Villapastel.", aprendizChef: "Aprendiste a robar cerveza con Bortrand.", protegisteCaido: "Tu primer domingo protegiste a un compañero caído en vez de buscar el gol.", prometisteSobrevivir: "Te prometiste algo modesto: sobrevivir la temporada entera. Y aquí sigues, contándolo.", vengasteLaPalmada: "Le devolviste al carnicero su palmada 'de buen rollo', ante sesenta mil personas.", enterrasteALaAbuela: "Amasaste su última tarta y la dejaste enfriándose en la ventana, por si pasaba algún carro.", fichastePorMortaigne: "Vestiste la camiseta negra de Mortaigne, y aprendiste a ganar como ganan los grandes.", envenenasteConMortaigne: "Echaste el cazo en la olla del rival, y esa noche no soñaste con nada.", teNegasteALaOlla: "Te ofrecieron el cazo del veneno en Mortaigne, y lo devolviste lleno.", volcasteLaOlla: "Volcaste la olla de Mortaigne de una patada, y te ganaste un enemigo con nombre.", camisetaEscondida: "En Mortaigne llevaste la camiseta de los pasteles debajo de la negra, donde nadie la viera.", volvisteDeMortaigne: "Volviste de Mortaigne al barro que huele a estofado, con la camiseta negra en el fondo de una bolsa." },
};

// Transiciones del halfling: la voz de la Comarca entre capítulo y capítulo.
const HALFLING_TRANSICIONES = {
  2: (pj) => `Pasa un invierno de harina y frío. Firmas con los Comepasteles de Villapastel, que en la Comarca es como firmar por no ir a ninguna parte. ${pj.flags.firmasteAPipo ? "Pipo Cazuelas ya te descuenta la primera tarta del sueldo, y aún no has cobrado." : "Tu abuela guarda el contrato en la lata de las galletas, que es donde guarda lo importante."} Roblerto sigue en su charca, mirando cosas redondas. Bortrand afila los cuchillos de la cocina, que aquí es afilar el equipo. La Sexta División te espera, si a eso se le puede llamar esperar.`,
  3: (pj) => `Pasan dos temporadas de perder con dignidad y comer con ganas. ${pj.flags.anotasteElPrimero ? "Aquel touchdown tuyo, el primero que anota un halfling en Villapastel, aún se cuenta en las cocinas, cada vez más adornado." : "Nadie recuerda que ganarais nada, pero recuerdan bien lo que se comió en la grada."} ${pj.flags.amigoArbol ? "Roblerto ya te distingue del balón por el olor a mantequilla, casi siempre." : "Roblerto todavía duda, a veces, si lanzarte a ti o al balón."} Y entonces empieza el lío del árbol, que va a dar que hablar hasta en la federación.`,
  4: (pj) => `${pj.flags.roblertoVendido ? "Sin Roblerto, que acabó de atracción en un circo de Tercera, jugáis como once pasteles corriendo. Pero al menos ya nadie os come al mediocampo." : pj.flags.roblertoEscupe ? "Lo del árbol que se come árbitros y los escupe vivos os ha hecho famosos por el motivo equivocado, que en un halfling es el único motivo posible." : "Roblerto sigue con vosotros, con expediente abierto y todo, que es lo más cerca de una estrella que tendréis nunca."} La Comarca ya sabe tu nombre, y eso, en un sitio donde nadie se va, es peligroso: significa que alguien de fuera también lo ha oído.`,
...
const HALFLING_ENTREACTOS = [
  ...
  { id: "esquivarRoblerto", caps: [2, 3, 4], txt: "Entrenar esquivas dejando que Roblerto intente lanzarte.", req: { rel: ["arbol", 1], noflag: "roblertoVendido" }, fx: { stat: { AG: 1 }, rel: { arbol: 1 } }, msg: "Roblerto te confunde con el balón y te lanza. Aprendes a caer rodando y a oler cuándo va a agacharse. Es el entrenamiento de agilidad más peligroso de la Comarca." },
...
const TENTACION = {
  ...
  halfling: { nombre: "Ganar como los grandes", flag: "cedisteALaTentacion", test: (pj) => pj.flags.fichastePorMortaigne || pj.flags.roblertoVendido || pj.flags.envenenasteLaFinal },
};
...
const rollKey = (pj, m, o) => {
  ...
  const mod = modDe(pj, o.stat) + clima + (pj.formaPend || 0) + (m.fatiga >= 3 ? -1 : 0) + (pj.flags.apaleado ? -1 : 0) + (o.bonus || 0);
...
const muerteDefinitiva = (raza) => raza === "halfling" || raza === "orco";
...
const marcadorFrase = (m) => { const [a, b] = m.marcador; if (a === b) return a === 0 ? varia(["seguís sin mojar, 0-0", "el marcador sigue a cero", "todavía nadie ha marcado", "0-0 y subiendo la tensión"], m, 5) : `vais igualados a ${numTxt(a)}`; return a > b ? `vais por delante ${a}-${b}` : `perdéis ${a}-${b}`; };
...
const PLAY_POOL = {
  saque: (pj, m) => ({ etq: "Saque", h: varia(["La bola en tierra de nadie", "El saque, al aire", "Primer balón, primer barro", "A por la vejiga"], m, 1),
    ...
    ops: pickN([
      ...
      { txt: "Recogerla y lanzarla larga a un compañero solo", det: "La bola vuela mejor de lo que corres tú.", stat: "AG", obj: 9, hab: "Pasar",
        ok: { txt: "La recoges del barro y la lanzas por encima de todos; cae en manos de un tuyo que ya corría. Vuestra y avanzada.", posesion: "propia", pase: true },
        ko: { txt: "El lanzamiento se va largo y lo baja un rival de un manotazo. La cazan ellos.", posesion: "rival" } },
  ...
  ataque: (pj, m) => m.posesion === "propia" ? ({ etq: "Ataque", h: varia(["Tienes la bola", "La bola es tuya", "Toca atacar", "El hueco espera"], m, 1),
    situ: `${varia([`Avanzáis. La caja de ${m.rivalCorto} se cierra a cuatro pasos de su línea; el hueco se abre y se cierra.`, `Tenéis la bola y campo por delante. ${m.rivalCorto} recula, ordena su muro y espera el error.`, `La bola pegada al pecho, la línea de gol a la vista y ${m.rivalCorto} entre medias. Ahora o nunca.`], m, 2)} ${varia([``, flavRival(m)], m, 4)}`.trim(),
    ops: pickN([
      { txt: "Pase a un compañero que está solo", det: "Ver el hueco antes de que exista.", stat: "AG", obj: 9, hab: "Pasar",
        ok: { txt: "Sueltas un pase tenso que cruza el aire y cae en manos amigas al otro lado. ¡Cruza! Touchdown.", gol: true, pase: true },
      ...
      { txt: "Esquivar al primero que sale y arrancar solo", det: "Un quiebro y a correr.", stat: "AG", obj: 9, riesgo: true, hab: "Esquivar",
        ok: { txt: "Esquivas al primero que se lanza a placarte, la caja se descoloca para taparte, y sales por el hueco que dejan, solo hasta la línea. ¡Touchdown!", gol: true },
        ko: { txt: "No esquivas limpio: te enganchan y te tiran al barro. La bola cambia de manos. Contraataque.", posesion: "rival" } },
  ...
  regate: (pj, m) => ({ etq: "Baile", h: varia(["Pasar sin chocar", "El baile", "Piernas, no hombro", "Entre líneas"], m, 1),
    ...
    ops: pickN([
      { txt: "Esquivar entre dos y salir por el hueco", det: "Donde ellos no están.", stat: "AG", obj: 8, riesgo: false, hab: "Esquivar",
        ok: { txt: "Pasas entre los dos como si no estuvieran y sales con la bola pegada al pecho. Vuestra.", posesion: "propia" },
      ...
      { txt: "Pase largo por encima de la caja", det: "La bola vuela donde tú no llegas.", stat: "AG", obj: 9, hab: "Pasar",
        ok: { txt: "La cuelgas por encima de todos y cae en manos amigas al otro lado. Vuestra, y avanzada.", posesion: "propia", pase: true },
      ...
      { txt: "Amagar a un lado y salir por el otro", det: "El cuerpo miente, los pies no.", stat: "AG", obj: 8, hab: "Esquivar",
        ok: { txt: "Cargas el peso a la derecha, el defensa se lo traga entero, y sales por la izquierda con la bola pegada al pecho. Se queda mirando tu sombra. Vuestra.", posesion: "propia" },
        ko: { txt: "Amagas demasiado y te enredas tú solo los pies. La bola se va, y ellos detrás.", posesion: "rival" } },
      { txt: "Saltar por encima del que se lanza a placarte", det: "Por arriba, donde no llegan.", stat: "AG", obj: 9, riesgo: true, hab: "Saltar",
        ok: { txt: "El grandote se lanza a tus piernas y tú saltas por encima con la bola pegada al pecho; caes al otro lado sin frenar y sales corriendo mientras la grada ruge. Vuestra, y con estilo.", posesion: "propia" },
        ko: { txt: "Saltas tarde y te caza en el aire por una pierna. Caes de espaldas y la bola rueda hacia ellos.", posesion: "rival" } },
    ], m) }),
  ...
  remate: (pj, m) => ({ etq: "Remate", h: varia(["A las puertas", "El último paso", "A un palmo del gol", "La jugada de gol"], m, 1),
    ...
    ops: pickN([
      { txt: "Pase a la esquina, donde no llega nadie", det: "La jugada lista.", stat: "AG", obj: 9, hab: "Pasar",
...
/* ===== POOL DEL HALFLING: MISMO MOTOR, OTRAS ACCIONES (comedia negra) =====
   Los Comepasteles no placan: se esconden, ruedan, dejan que Roblerto (el árbol)
...
// Confianza de Roblerto (rel.arbol, la "mini-relación" del cliente): pesa en
// sus jugadas, en la narración y en la tirada. Buena: te reconoce y va a la
// primera (+1). Mala: no se fía ni de ti ni del balón (−1).
const confRob = (pj) => { const c = (pj.rel && pj.rel.arbol) || 0; return c >= 2 ? { b: 1, sabor: "Te reconoce por el olor a mantequilla y va a la primera." } : c <= -1 ? { b: -1, sabor: "Te mira. Mira el balón. Te mira otra vez. Decide que los dos sois sospechosos." } : { b: 0, sabor: "" }; };
// ¿Ya no está Roblerto? (lo vendiste, o te fuiste a Mortaigne). Sus jugadas de
// partido se sustituyen por variantes "sin árbol": lo que hace un equipo de
// halflings que se quedó sin su gigante — más pobre y más peligroso, para que la
// ausencia se note en cada partido (biblia: la venta debe doler todo el libro).
const sinRoblerto = (pj) => !!(pj.flags && (pj.flags.roblertoVendido || pj.flags.fichastePorMortaigne));
    ...
    ops: pickN([
      ...
      sinRoblerto(pj)
        ? { txt: "Lanzarte a por ella entre las botas, a pelo", det: "Sin árbol, solo tú y un bosque de piernas.", bonus: -1, stat: "AG", obj: 8, riesgo: true, hab: "Esquivar",
            ...
            ko: { txt: "Sin el árbol que mirar, todas esas botas te miran a ti. Una te encuentra. La bola se queda con ellos y tú con el barro.", posesion: "rival" } }
        : { txt: "Dejar que Roblerto la coja creyendo que es un pastel", det: confRob(pj).sabor || "El árbol ayuda. A su manera.", bonus: confRob(pj).b, stat: "ST", obj: 8, riesgo: true, hab: "Placar",
            ok: { txt: "Roblerto agarra la bola pensando que es de comer, se decepciona, y la deja caer justo en tus manos. Gracias, Roblerto. Vuestra.", posesion: "propia" },
            ko: { txt: "Roblerto te agarra a TI pensando que eres la bola. Descubres el mundo desde diez metros antes de aterrizar en el barro. Ellos aprovechan y se la llevan.", posesion: "rival" } },
      sinRoblerto(pj)
        ? { txt: "Colártela a pelo, sin distracción que valga", det: "Ya no hay árbol al que mirar. Solo tú y su defensa.", bonus: -1, stat: "AG", obj: 9, hab: "Manos seguras",
            ok: { txt: "Ya no hay árbol que distraiga a nadie: solo tú, el balón y toda su defensa mirándote. Lo haces a la vieja usanza —rápido y con suerte— y sale. Vuestra.", posesion: "propia" },
            ko: { txt: "A la vieja usanza también se falla a la vieja usanza. Te ven venir de lejos y la cazan ellos.", posesion: "rival" } }
    ...
    ops: pickN([
      ...
      sinRoblerto(pj)
        ? { txt: "Trepar por tus propios compañeros amontonados", det: "Sin árbol, la escalera sois vosotros.", bonus: -1, stat: "MA", obj: 9, riesgo: true, hab: "Esprintar",
            ...
            ko: { txt: "El montón se desmorona a mitad de trepada y caéis todos como bolos, tú el primero. Ellos recogen la bola de entre el revoltijo.", posesion: "rival" } }
    ...
    ops: pickN([
      ...
      sinRoblerto(pj)
        ? { txt: "Amontonaros todos y empujar como uno", det: "Sin tronco, la fuerza es la unión (poca, pero toda).", bonus: -1, stat: "ST", obj: 8, riesgo: true, hab: "Mantenerse firme",
            ...
            ko: { txt: "Empujáis todos a la vez y no se mueve nada, salvo vosotros, hacia atrás. Sin el árbol, un choque de frente es lo que siempre fue para un halfling: una mala idea.", posesion: "rival" } }
        : { txt: "Poner a Roblerto por delante y esconderte detrás", det: confRob(pj).sabor || "El árbol es tu muro.", bonus: confRob(pj).b, stat: "ST", obj: 8, riesgo: true, hab: "Placar",
            ok: { txt: "Empujas a Roblerto hacia la línea rival y te pegas a su tronco. El árbol arrolla lo que pilla y tú vas detrás, a salvo. El barro es vuestro.", posesion: "propia" },
            ko: { txt: "Roblerto decide sentarse justo cuando lo necesitas de pie, y te quedas solo delante del muro. El resultado es predecible y con crujido.", posesion: "rival" } },
  ...
  regate: (pj, m) => ({ etq: "Baile", h: varia(["Pasar sin que te toquen", "Lo que mejor sabes hacer", "Escurrirse entre gigantes"], m, 1),
    ...
    ops: pickN([
      { txt: "Escurrirte entre dos por el hueco de abajo", det: "Donde no te ven.", stat: "AG", obj: 8, hab: "Esquivar",
        ok: { txt: "Pasas entre dos gigantes por el hueco de sus rodillas, tan tranquilo, con la bola pegada al pecho. Ni se enteran. Vuestra.", posesion: "propia" },
      ...
      sinRoblerto(pj)
        ? { txt: "Pasar en corto entre los tuyos, sin torres", det: "Sin árbol que despeje, la bola va de pie en pie.", bonus: -1, stat: "AG", obj: 9,
            ...
            ko: { txt: "Un pase en corto de más y un grandote mete la bota en medio. Sin Roblerto para despejar lejos, la pérdida se paga cerca. Para ellos.", posesion: "rival" } }
  ...
  remate: (pj, m) => ({ etq: "Remate", h: varia(["A las puertas (agárrate)", "El último palmo", "La jugada del pastel"], m, 1),
    ...
    ops: pickN([
      { txt: "El pase imposible por debajo de todos", det: "Nadie mira a ras de barro.", stat: "AG", obj: 9, hab: "Esquivar",
        ok: { txt: "Sueltas un pase raso que cruza el barro por debajo de doce entrepiernas y cae en manos amigas al otro lado. Nadie mira nunca a la altura de un halfling. ¡Touchdown!", gol: true, pase: true },
        ko: { txt: "El pase raso choca contra una espinilla del tamaño de un tronco y rebota hacia ellos. Contraataque.", posesion: "rival" } },
      sinRoblerto(pj)
        ? { txt: "Que tres compañeros te lancen a pulso, sin red", det: "Sin el árbol, la catapulta sois vosotros.", bonus: -1, stat: "AG", obj: 10, riesgo: true, hab: "Esquivar",
            ok: { txt: "Sin árbol, la catapulta sois vosotros: seis manos pequeñas te agarran, te lanzan y rezan. Es la mitad de alto y el doble de peligroso, pero cruzas la línea por los pelos. ¡Touchdown a pulso!", gol: true },
            ko: { txt: "Seis manos pequeñas no dan para tanto: te lanzan, sí, pero corto y torcido, y aterrizas de bruces del lado malo. Sin gol, con barro en la boca.", posesion: "rival" } }
        : { txt: "Que Roblerto te lance por encima de la línea", det: confRob(pj).sabor || "La bomba halfling.", bonus: confRob(pj).b, stat: "ST", obj: 10, riesgo: true, hab: "Placar",
            ok: { txt: "Te pones en la mano de Roblerto, gritas '¡ahora!', y el árbol te lanza en un arco perfecto por encima de todo el muro. Aterrizas al otro lado de la línea con la bola. La grada enloquece. ¡Touchdown volando!", gol: true },
            ko: { txt: "Roblerto te lanza, sí, pero calcula fatal: acabas en la tercera fila de la grada, entre dos aficionados que comparten su bocadillo contigo. Sin gol, y con acidez.", posesion: "rival" } },
    ...
    ops: pickN([
      ...
      sinRoblerto(pj)
        ? { txt: "Hacer una muralla de halflings en la línea", det: "Doce cuerpos pequeños donde antes había un tronco.", bonus: (pj.rel && pj.rel.equipo >= 3 ? 0 : -1), stat: "ST", obj: 9, riesgo: true, hab: "Mantenerse firme",
            ...
            ko: { txt: "La muralla de pasteles aguanta un empujón, dos, y al tercero se abre por el medio. Sin el árbol, un halfling no para a un grandote lanzado. Touchdown suyo.", golRival: true } }
...
const cumple = (pj, req) => {
  ...
  for (const [k, v] of Object.entries(req)) {
    ...
    // Requisito de eje (biblia §18.3, orco): umbral positivo = mínimo (eje ≥ n);
    // negativo = máximo (eje ≤ n). Así los finales leen dónde quedó cada tensión.
    else if (k === "eje") { for (const [e, n] of Object.entries(v)) { const val = (pj.eje || {})[e] || 0; if (n >= 0 ? val < n : val > n) return false; } }
...
const textoReq = (req, REL) => Object.entries(req).map(([k, v]) => {
  ...
  if (k === "eje") { const et = { bandaCharca: "el camino de la charca", jefeIgual: "haber mandado como jefe", deudaLibre: "la deuda con Ma Gorka", enteraSolo: "haber conservado a los tuyos" }; return Object.keys(v).map((e) => et[e] || e).join(", "); }
  return `${k} ${v}`;
}).join(", ");
...
export default function App() {
  ...
  const tirarHerida = (q, m) => {
    ...
    if (arm >= avEf) {
      ...
      if (her <= 7) texto = " " + pick1(HERIDA_PROSA.aturdido());
        ...
        else if (les <= (muerteDefinitiva(q.raza) ? 15 : 14)) { const d = d6(); const st = d <= 2 ? "MA" : d <= 4 ? "AV" : d === 5 ? "AG" : "ST"; q[st] = Math.max(1, q[st] - 1); chips.push(`Herida permanente: −1 ${st}`); texto = ` Herida permanente. ${HERIDA_PROSA.permanente[st]}`; }
  ...
  const jugarJugada = (o) => {
    ...
    if (muerte) {
      ...
      // Razas migradas (halfling, orco…) no resucitan: la primera muerte es la
      // definitiva, con el epitafio de su etapa. Las demás conservan MAX_MUERTES.
      const tope = muerteDefinitiva(q.raza) ? 0 : MAX_MUERTES;
      if (n > tope) {
        const etapa = etapaMuerte(q.raza, ORDEN[idx].cap);
        setPj({ ...q, muertes: n }); setMt(null);
        setMuerteInfo(muerteDefinitiva(q.raza) ? { final: true, titulo: "No te levantas", texto: [comoCayo, etapa].filter(Boolean).join(" ") } : null);
  ...
  const continuar = () => {
    if (panel?.muerte) {
      ...
      const tope = muerteDefinitiva(pj.raza) ? 0 : MAX_MUERTES;
      if (n > tope) {
        const etapa = etapaMuerte(pj.raza, ORDEN[idx].cap);
        setPj((p) => ({ ...p, muertes: n }));
        setMuerteInfo(muerteDefinitiva(pj.raza) ? { final: true, titulo: "No te levantas", texto: etapa } : null);
    ...
    <div className="libro" role="region" aria-label="Libro del destino">
      ...
      <div className="col2">
        <div>
          ...
          <p className="mini">{muerteDefinitiva(pj.raza) ? "Sin resucitar: si caes, se acabó" : `Muertes: ${pj.muertes} de ${MAX_MUERTES}`}</p>
